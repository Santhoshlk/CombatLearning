...
void UEnemyCombatComponent::OnWeaponHitTarget(AActor* HitActor)
{
	...
	bool bIsUnBlockable = UMorrowBoneFunctionLibrary::NativeDoesActorHaveTag(
		GetOwningPawn(),
		MorrowBoneGameplayTags::Enemy_Attack_UnBlockable);


	// is player Blocking
	bIsPlayerBlocking = UMorrowBoneFunctionLibrary::BP_DoesActorHaveTag(
		HitActor,
...
void UEnemyCombatComponent::ToggleBodyWeaponCollision(bool ActivateWeaponCollision, EWeaponEquippedTypes EquippedWeapon)
{
	...
	case EWeaponEquippedTypes::LeftHanded:
		ActivateWeaponCollision
			? LeftCollisionBox->SetCollisionEnabled(ECollisionEnabled::QueryOnly)
			: LeftCollisionBox->SetCollisionEnabled(ECollisionEnabled::NoCollision);
		break;

	case EWeaponEquippedTypes::RightHanded:
		ActivateWeaponCollision
			? RightCollisionBox->SetCollisionEnabled(ECollisionEnabled::QueryOnly)
