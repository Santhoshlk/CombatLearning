...
void UMBHeroGameplayAbility_TargetLock::GetAvailableTargets()
{
	...
	const FVector EndTrace = GetMorrowBoneCharacter()->GetActorLocation() + GetMorrowBoneCharacter()->
...
{
	if (!CurrentTargetLockActor || (UMorrowBoneFunctionLibrary::BP_DoesActorHaveTag(
			CurrentTargetLockActor,
			MorrowBoneGameplayTags::Shared_Status_Death))
		|| (UMorrowBoneFunctionLibrary::BP_DoesActorHaveTag(
			GetMorrowBoneCharacter(),
			MorrowBoneGameplayTags::Shared_Status_Death)))
	{
...
void UMBHeroGameplayAbility_TargetLock::SetRotationToCurrentLockedActor(float DeltaTime)
{
	...
	if (UMorrowBoneFunctionLibrary::BP_DoesActorHaveTag(
		GetMorrowBoneCharacter(),
		MorrowBoneGameplayTags::Player_Status_Rolling) || UMorrowBoneFunctionLibrary::BP_DoesActorHaveTag(
		GetMorrowBoneCharacter(),
	...
	if (bShouldInterpolate)
	{
		...
		const FRotator TargetRotation = UKismetMathLibrary::FindLookAtRotation(
			GetMorrowBoneCharacter()->GetActorLocation(),
			CurrentTargetLockActor->GetActorLocation());
		const FRotator InterpRotation = FMath::RInterpTo(
			GetMorrowBonePlayerController()->GetControlRotation(),
			TargetRotation,
			DeltaTime,
			RotationInterpSpeed);
                                                                 ...
                                                                 TArray<AActor*>& RightActors)
{
	...
	const FVector PlayerToCurrentEnemyNorm = (CurrentTargetLockActor->GetActorLocation() - GetMorrowBoneCharacter()->
	...
	for (AActor* AvailableActors : TraceOutHitActors)
	{
		...
		const FVector PlayerToEnemyNorm = (AvailableActors->GetActorLocation() - GetMorrowBoneCharacter()->
			GetActorLocation()).GetSafeNormal();
...
void UMBHeroGameplayAbility_TargetLock::CreateTargetLockWidget()
{
	...
	if (!TargetLockWidget)
	{
		TargetLockWidget = Cast<UMorrowBoneWidgetBase>(
			CreateWidget(GetMorrowBonePlayerController(), TargetLockWidgetClass));
