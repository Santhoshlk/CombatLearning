...
UAbilityTask_OnWaitSpawnEnemies* UAbilityTask_OnWaitSpawnEnemies::WaitSpawnEnemies(UGameplayAbility* OwningAbility,
	TSoftClassPtr<AEnemyBase> EnemiesSpawnClass, FGameplayTag InputTag, int32 NumToSpawn, const FVector& SpawnOrigin,
	float RadiusToSpawn
)
{
...
void UAbilityTask_OnWaitSpawnEnemies::OnEnemyAsyncLoad()
{
	...
	if (LoadedClass && LoadedWorld)
	{
		...
		for (int i = 0; i < CachedNumToSpawn; i++)
		{
			...
			const FRotator SpawnRotation = AbilitySystemComponent->GetAvatarActor()->GetActorForwardVector().
			                                                       ToOrientationRotator();

			AEnemyBase* SpawnedEnemy = LoadedWorld->SpawnActor<AEnemyBase>(
				LoadedClass,
				RandomLocation,
				SpawnRotation,
				Parameters);
